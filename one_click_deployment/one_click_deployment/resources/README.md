...
The databricks CLI bundles top level is defined by file `one_click_deployment/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`one_click_deployment/resources/model-workflow-resource.yml`)
 - batch inference workflow (`one_click_deployment/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`one_click_deployment/resources/monitoring-resource.yml`)
 - feature engineering workflow (`one_click_deployment/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`one_click_deployment/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `one_click_deployment/databricks.yml` and `one_click_deployment/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-one_click_deployment-model     | /dev-one_click_deployment-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-one_click_deployment-model | /staging-one_click_deployment-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-one_click_deployment-model    | /prod-one_click_deployment-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-one_click_deployment-model    | /test-one_click_deployment-experiment    |
...
2. Complete the `TODO` in `one_click_deployment/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `one_click_deployment/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `one_click_deployment/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-one_click_deployment-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-one_click_deployment-batch-inference-job`
that runs the notebook under `one_click_deployment/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-one_click_deployment-batch-inference-job`.
...
`test-one_click_deployment-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-one_click_deployment-batch-inference-job`.
